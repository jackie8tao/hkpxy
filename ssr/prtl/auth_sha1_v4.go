...
func (a *authSHA1v4) SetServerInfo(s *spt.ServerInfoForObfs) {
	a.ServerInfoForObfs = *s
}

func (a *authSHA1v4) GetServerInfo() (s *spt.ServerInfoForObfs) {
...
func (a *authSHA1v4) packData(data []byte) (outData []byte) {
	...
	crc32 := spt.CalcCRC32(outData, 2, 0xFFFFFFFF)
	...
	adler := spt.CalcAdler32(outData[:outLength-4])
...
func (a *authSHA1v4) packAuthData(data []byte) (outData []byte) {
	...
	outLength := dataOffset + dataLength + 12 + spt.ObfsHMACSHA1Len
	outData = make([]byte, outLength)
	...
	crc32 := spt.CalcCRC32(crcData, len(crcData), 0xFFFFFFFF)
	// 2~6, crc of out length+salt+key
	...
	h := spt.HmacSHA1(key, outData[:outLength-spt.ObfsHMACSHA1Len])
	// out length-10~out length/rand length+18+data length~end, hmac
	copy(outData[outLength-spt.ObfsHMACSHA1Len:], h[0:spt.ObfsHMACSHA1Len])
...
func (a *authSHA1v4) PreEncrypt(plainData []byte) (outData []byte, err error) {
	...
	if !a.hasSentHeader && dataLength > 0 {
		...
		if headSize := spt.GetHeadSize(plainData, 30); headSize <= dataLength {
			authLength = headSize
		}
...
func (a *authSHA1v4) PostDecrypt(plainData []byte) (outData []byte, err error) {
	...
	for a.recvBufferLength > 4 {
		crc32 := spt.CalcCRC32(a.recvBuffer, 2, 0xFFFFFFFF)
		if binary.LittleEndian.Uint16(a.recvBuffer[2:4]) != uint16(crc32&0xFFFF) {
			log.Error("auth_sha1_v4 post decrypt data crc32 error")
			return nil, spt.ErrAuthSHA1v4CRC32Error
		}
		length := int(binary.BigEndian.Uint16(a.recvBuffer[0:2]))
		if length >= 8192 || length < 8 {
			log.Error("auth_sha1_v4 post decrypt data length error")
			a.recvBufferLength = 0
			a.recvBuffer = nil
			return nil, spt.ErrAuthSHA1v4DataLengthError
		}
		if length > a.recvBufferLength {
			break
		}

		if spt.CheckAdler32(a.recvBuffer, length) {
			...
			log.Error("auth_sha1_v4 post decrypt incorrect checksum")
			a.recvBufferLength = 0
			a.recvBuffer = nil
			return nil, spt.ErrAuthSHA1v4IncorrectChecksum
		}
	}
	return
}
