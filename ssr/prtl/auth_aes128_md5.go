...
func (a *authAES128) SetServerInfo(s *spt.ServerInfoForObfs) {
	a.ServerInfoForObfs = *s
}

func (a *authAES128) GetServerInfo() (s *spt.ServerInfoForObfs) {
...
func (a *authAES128) packAuthData(data []byte) (outData []byte) {
	...
	if len(params) >= 2 {
		if userID, err := strconv.ParseUint(params[0], 10, 32); err != nil {
			log.Warning("parsing uint failed", params[0], err)
			rand.Read(uid)
		} else {
	...
	aesCipherKey := spt.EVPBytes2Key(base64.StdEncoding.EncodeToString(encryptKey)+a.salt, 16)
	block, err := aes.NewCipher(aesCipherKey)
	if err != nil {
		log.Error("creating aes cipher failed", err)
		return nil
	}
...
func (a *authAES128) PostDecrypt(plainData []byte) (outData []byte, err error) {
	...
	for a.recvBufferLength > 4 {
		...
		if h[0] != a.recvBuffer[2] || h[1] != a.recvBuffer[3] {
			log.Error("client post decrypt hmac error")
			return nil, spt.ErrAuthAES128HMACError
		}

		length := int(binary.LittleEndian.Uint16(a.recvBuffer[0:2]))
		if length >= 8192 || length < 8 {
			log.Error("client post decrypt length mismatch")
			return nil, spt.ErrAuthAES128DataLengthError
		}
		...
		if !hmac.Equal(h[0:4], a.recvBuffer[length-4:]) {
			log.Error("client post decrypt incorrect checksum")
			return nil, spt.ErrAuthAES128IncorrectChecksum
		}
