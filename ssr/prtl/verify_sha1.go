...
func (v *verifySHA1) otaConnectAuth(data []byte) []byte {
	return append(data, spt.HmacSHA1(append(v.IV, v.Key...), data)...)
}
...
func (v *verifySHA1) otaReqChunkAuth(chunkId uint32, data []byte) []byte {
	...
	header := append(nb, spt.HmacSHA1(append(v.IV, chunkIdBytes...), data)...)
...
func (v *verifySHA1) otaVerifyAuth(iv []byte, chunkId uint32, data []byte, expectedHmacSha1 []byte) bool {
	chunkIdBytes := make([]byte, 4)
	binary.BigEndian.PutUint32(chunkIdBytes, chunkId)
	actualHmacSha1 := spt.HmacSHA1(append(iv, chunkIdBytes...), data)
...
func (v *verifySHA1) SetServerInfo(s *spt.ServerInfoForObfs) {
	v.ServerInfoForObfs = *s
}

func (v *verifySHA1) GetServerInfo() (s *spt.ServerInfoForObfs) {
	return &v.ServerInfoForObfs
}

func (v *verifySHA1) SetData(_ interface{}) {
